...
class HumanModeService:
    def __init__(self):
        self.config = get_mode_config('human')
    ...
    def process_signal(self, signal_data: np.ndarray, scales: List[float], 
                      use_wavelet: bool = False, wavelet_type: str = 'haar'):
        """Process signal with human mode equalization"""
        if use_wavelet:
            # Wavelet-based processing (default haar for human mode)
