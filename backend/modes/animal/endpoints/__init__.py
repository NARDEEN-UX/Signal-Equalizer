"""Animal mode endpoints"""
from fastapi import APIRouter, HTTPException
from typing import List
import numpy as np
from ..services import AnimalModeService
from ..schemas import AnimalEqualizationRequest

router = APIRouter(prefix="/modes/animal", tags=["animal"])

current_signal = None
sample_rate = None
service = AnimalModeService()

@router.get("/info")
async def get_animal_mode_info():
    """Get animal mode information with requirements"""
    return service.get_mode_info()

@router.post("/process")
async def process_animal_equalization(request: AnimalEqualizationRequest):
    """Process signal with animal mode equalization"""
