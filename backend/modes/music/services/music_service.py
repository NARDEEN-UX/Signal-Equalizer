...
class MusicModeService:
    ...
    def _combine_overlap_gains(matched_gains: List[float]) -> float:
        """
        Combine gains for overlapping controls while avoiding full cancellation.

        Rules:
        - no matched bands -> neutral (1.0)
        - matched active bands (gain > 0) -> arithmetic mean over all matched bands
          (including muted overlaps), to preserve attenuation from overlap logic
        - matched bands but all muted -> 0.0
        """
    ...
    def _apply_instrument_equalization(
        ...
        if mask_rows:
            ...
            # Mean gain over all matched controls keeps overlap attenuation while
            # still preventing full cancellation unless all overlaps are muted.
    ...
    def separate_with_demucs(
        ...
        # Demucs expects tensor shape [batch, channels, time].
    ...
    def _estimate_frequency_range(signal: np.ndarray, sample_rate: int) -> Tuple[float, float]:
        ...
        # Avoid pathological narrow bands in sparse stems.
        if high - low < 40.0:
            center = (low + high) / 2.0
    ...
    def _normalize_demucs_output(estimated_raw, source_count: int):
        """
        Normalize Demucs outputs to shape [sources, channels, time].
        Supports tensor layouts from different demucs/torch versions.
        """
        estimated = estimated_raw[0] if isinstance(estimated_raw, (list, tuple)) else estimated_raw

        if getattr(estimated, "ndim", None) == 4:
            # Common shape: [batch, sources, channels, time].
        ...
        # Expected [sources, channels, time]. If sources are on axis=1, transpose.
