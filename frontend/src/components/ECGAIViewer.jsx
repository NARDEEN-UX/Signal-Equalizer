...
function CamTimelineCanvas({ camTimeline }) {
  const canvasRef = useRef(null);
  useEffect(() => {
    if (!canvasRef.current || !camTimeline?.length) return;
    const canvas = canvasRef.current;
    const W = canvas.clientWidth || 600;
    const H = 80;
    canvas.width = W; canvas.height = H;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = '#0f172a'; ctx.fillRect(0,0,W,H);
    const maxV = Math.max(...camTimeline, 1e-8);
    ctx.beginPath(); ctx.strokeStyle = '#818cf8'; ctx.lineWidth = 1.5;
    camTimeline.forEach((v,i) => {
      const x = (i/(camTimeline.length-1))*W;
      const y = H - (v/maxV)*(H-4) - 2;
      i === 0 ? ctx.moveTo(x,y) : ctx.lineTo(x,y);
    });
    ctx.stroke();
    ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath();
    ctx.fillStyle = 'rgba(129,140,248,0.15)'; ctx.fill();
  }, [camTimeline]);
  return <canvas ref={canvasRef} style={{ width:'100%', height:'80px', borderRadius:'6px', display:'block' }} />;
}
...
function WindowStrip({ windows }) {
  ...
  return (
    <div style={{ position:'relative', height:'32px', background:'#1e293b', borderRadius:'6px', overflow:'hidden' }}>
      {windows.map((w,i) => {
...
function TimeImportanceDots({ timeImportance, duration }) {
  ...
  return (
    <div style={{ position:'relative', height:'52px', background:'#1e293b', borderRadius:'6px', overflow:'hidden' }}>
  ...
  return (
    <div style={{ fontFamily:'monospace', fontSize:'11px', color:'#94a3b8', background:'#0f172a', borderRadius:'6px', padding:'10px 12px', lineHeight:'1.7' }}>
      ...
      <div style={{ marginBottom:'6px' }}>
        {CLASS_ORDER.map(cls => (
          <div key={cls} style={{ display:'flex', gap:'8px' }}>
            <span style={{ width:'110px', color:CLASS_COLORS[cls] }}>{cls}</span>
            <span>{Number(result.probabilities?.[cls]||0).toFixed(2)}%</span>
          </div>
        ))}
      </div>
...
// ═════════════════════════════════════════════════════════════════════════════
export default function ECGAIViewer({ signal, sampleRate }) {
        ...
        <>
          ...
          <div style={{ background:'rgba(255,255,255,0.04)', border:`1px solid ${CLASS_COLORS[result.pred_label]||'#6366f1'}44`, borderRadius:'10px', padding:'14px 16px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'12px' }}>
              ...
              <div>
                <span style={{ fontSize:'22px', fontWeight:800, color:'#f1f5f9' }}>{Number(result.confidence).toFixed(1)}%</span>
          ...
          {result.is_long && (
            <>
              <Section title="Averaged GradCAM Timeline">
                <CamTimelineCanvas camTimeline={result.cam_timeline} />
              </Section>
              ...
              <Section title="Mean Probability Breakdown">
                <ProbBars probabilities={result.mean_probabilities} />
              </Section>
            </>
          )}
        </>
